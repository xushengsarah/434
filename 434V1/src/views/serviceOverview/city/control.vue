...
export default {
    ...
    methods: {
        ...
        initG6() {
            let self = this
            const width = document.getElementById('control').scrollWidth;
            const graph = new G6.Graph({
                container: 'control', // String | HTMLElement，必须，在 Step 1 中创建的容器 id 或容器本身
                ...
                draw: (cfg, group) => {
                    const shape = group.addShape('rect', {
                        attrs: {
                            ...
                            radius: 4,
                            fill: '#002343',
                            fillOpacity: cfg.labelCfg.fillOpacity ? 0 : .7,
                            stroke: (cfg.labelCfg.fillOpacity || cfg.tag)  ? 'transparent' : (cfg.style.stroke ? cfg.style.stroke : '#2193FE'),
                            lineWidth: 1,
                            label: cfg.label,
                            cursor: 'pointer',
                            // background: url('@/assets/img/serviceOverview/net_bg.png'),
                    ...
                    //         width: 100,
                    //         height: 100,
                    //         img: 'http://localhost:8899/assets/img/serviceOverview/alarm_animate.png',
                    //     }
                    // });
                    // 里面的那层
                    group.addShape('text', {
                        attrs: {
                            x:  (cfg.size[0]) / 2,
                            y: cfg.tag ? 18 : ( cfg.boundary ? cfg.size[1] / 2 + 8 : (cfg.size[1]) / 3.5 * 2),
                            text: cfg.label,
                            textAlign: 'center',
                            // 默认的标签配置
            ...
            graph.on('click', (ev) => {
                ...
                } else if (ev.item._cfg.id !== 'boundary') {
