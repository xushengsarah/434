...
export default {
    ...
    methods: {
        ...
        initG6() {
            let self = this
            const width = document.getElementById('dynamicSearch').scrollWidth;
            const graph = new G6.Graph({
                container: 'dynamicSearch', // String | HTMLElement，必须，在 Step 1 中创建的容器 id 或容器本身
            ...
            G6.registerNode('rectNode', {
                ...
                draw: (cfg, group) => {
                    ...
                    group.addShape('text', {
                        attrs: {
                            x:  (cfg.size[0]) / 2,
                            y: cfg.tag ? 18 : ( cfg.boundary ? cfg.size[1] / 2 + 6 : (cfg.size[1]) / 3.5 * 2),
                            text: cfg.label,
                            textAlign: 'center',
                            // 默认的标签配置
